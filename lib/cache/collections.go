...
type collections struct {
	...
	provisionTokens                  *collection[types.ProvisionToken, provisionTokenStoreIndex]
	staticTokens                     *collection[types.StaticTokens, staticTokensStoreIndex]
	certAuthorities                  *collection[types.CertAuthority, certAuthorityStoreIndex]
	users                            *collection[types.User, userStoreIndex]
	roles                            *collection[types.Role, roleStoreIndex]
	authServers                      *collection[types.Server, authServerStoreIndex]
	proxyServers                     *collection[types.Server, proxyServerStoreIndex]
	nodes                            *collection[types.Server, nodeStoreIndex]
	apps                             *collection[types.Application, appStoreIndex]
	appServers                       *collection[types.AppServer, appServerStoreIndex]
	dbs                              *collection[types.Database, databaseStoreIndex]
	dbServers                        *collection[types.DatabaseServer, databaseServerStoreIndex]
	dbServices                       *collection[types.DatabaseService, databaseServiceStoreIndex]
	kubeServers                      *collection[types.KubeServer, kubeServerStoreIndex]
	kubeClusters                     *collection[types.KubeCluster, kubeClusterStoreIndex]
	windowsDesktops                  *collection[types.WindowsDesktop, windowsDesktopStoreIndex]
	windowsDesktopServices           *collection[types.WindowsDesktopService, windowsDesktopServiceStoreIndex]
	userGroups                       *collection[types.UserGroup, userGroupStoreIndex]
	identityCenterAccounts           *collection[*identitycenterv1.Account, identityCenterAccountStoreIndex]
	identityCenterAccountAssignments *collection[*identitycenterv1.AccountAssignment, identityCenterAccountAssignmentStoreIndex]
	healthCheckConfig                *collection[*healthcheckconfigv1.HealthCheckConfig, healthCheckConfigStoreIndex]
	reverseTunnels                   *collection[types.ReverseTunnel, reverseTunnelStoreIndex]
	spiffeFederations                *collection[*machineidv1.SPIFFEFederation, spiffeFederationIndex]
	workloadIdentity                 *collection[*workloadidentityv1.WorkloadIdentity, workloadIdentityStoreIndex]
