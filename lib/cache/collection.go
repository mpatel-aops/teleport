...
type collection[T any, I ~string] struct {
...
func (c *collection[T, I]) onDelete(r types.Resource) error {
	switch t := r.(type) {
...
//
// This is a no-op if the configured filter does not return true.
func (c *collection[T, I]) onPut(r types.Resource) error {
	switch t := r.(type) {
	case interface{ UnwrapT() T }:
		tt := t.UnwrapT()
		if c.filter != nil && !c.filter(tt) {
...
func (c collection[T, I]) fetch(ctx context.Context, cacheOK bool) (apply func(context.Context) error, err error) {
