...
const accessListStoreNameIndex = "name"

func newAccessListCollection(upstream services.AccessLists, w types.WatchKind) (*collection[*accesslist.AccessList], error) {
	if upstream == nil {
		return nil, trace.BadParameter("missing parameter AccessLists")
	}

	return &collection[*accesslist.AccessList]{
		store: newStore(map[string]func(*accesslist.AccessList) string{
			accessListStoreNameIndex: func(r *accesslist.AccessList) string {
				return r.GetName()
			},
		}),
...
func (c *Cache) GetAccessLists(ctx context.Context) ([]*accesslist.AccessList, error) {
	...
	for n := range rg.store.resources(accessListStoreNameIndex, "", "") {
...
func (c *Cache) ListAccessLists(ctx context.Context, pageSize int, pageToken string) ([]*accesslist.AccessList, string, error) {
	...
	lister := genericLister[*accesslist.AccessList]{
		cache:           c,
		collection:      c.collections.accessLists,
		index:           accessListStoreNameIndex,
		defaultPageSize: 100,
...
func (c *Cache) GetAccessList(ctx context.Context, name string) (*accesslist.AccessList, error) {
	...
	getter := genericGetter[*accesslist.AccessList]{
		cache:      c,
		collection: c.collections.accessLists,
		index:      accessListStoreNameIndex,
...
const (
	accessListMemberStoreNameIndex = "name"
	accessListMemberStoreKindIndex = "kind"
)

func newAccessListMemberCollection(upstream services.AccessLists, w types.WatchKind) (*collection[*accesslist.AccessListMember], error) {
	if upstream == nil {
		return nil, trace.BadParameter("missing parameter AccessLists")
	}

	return &collection[*accesslist.AccessListMember]{
		store: newStore(map[string]func(*accesslist.AccessListMember) string{
...
func (c *Cache) ListAccessListMembers(ctx context.Context, accessListName string, pageSize int, pageToken string) (members []*accesslist.AccessListMember, nextToken string, err error) {
	...
	for member := range rg.store.resources(accessListMemberStoreNameIndex, start, "") {
		if len(out) == pageSize {
...
func (c *Cache) ListAllAccessListMembers(ctx context.Context, pageSize int, pageToken string) (members []*accesslist.AccessListMember, nextToken string, err error) {
	...
	lister := genericLister[*accesslist.AccessListMember]{
		cache:           c,
		collection:      c.collections.accessListMembers,
		index:           accessListMemberStoreNameIndex,
		defaultPageSize: 200,
...
func (c *Cache) GetAccessListMember(ctx context.Context, accessList string, memberName string) (*accesslist.AccessListMember, error) {
	...
	member, err := rg.store.get(accessListMemberStoreNameIndex, accessList+"/"+memberName)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
const accessListReviewStoreNameIndex = "name"

func newAccessListReviewCollection(upstream services.AccessLists, w types.WatchKind) (*collection[*accesslist.Review], error) {
	if upstream == nil {
		return nil, trace.BadParameter("missing parameter AccessLists")
	}

	return &collection[*accesslist.Review]{
		store: newStore(map[string]func(*accesslist.Review) string{
			accessListReviewStoreNameIndex: func(r *accesslist.Review) string {
				return r.Spec.AccessList + "/" + r.GetName()
			},
		}),
...
func (c *Cache) ListAccessListReviews(ctx context.Context, accessList string, pageSize int, pageToken string) ([]*accesslist.Review, string, error) {
	...
	lister := genericLister[*accesslist.Review]{
		cache:           c,
		collection:      c.collections.accessListReviews,
		index:           accessListReviewStoreNameIndex,
		defaultPageSize: 200,
